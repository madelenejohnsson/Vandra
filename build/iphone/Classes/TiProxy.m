...
//Rare exceptions to indicate a bug in the _kostervandring code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_kostervandring = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_kostervandring]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.kostervandring.module"
